...
export default class RecentCommitsController extends React.Component {
  ...
  constructor(props, context) {
    ...
    this.state = { selectedCommitSha: "", maxCommits: 50 };
  }
  ...
  fetchData = (repository) => repository.getRecentCommits({ max: this.state.maxCommits });

  loadMoreCommits = () => this.setState((s) => ({ maxCommits: s.maxCommits + 50 }));
  ...
  render() {
    return (
      <ObserveModel
        ...
        fetchParams={[this.state.maxCommits]}
      >
        {(commits) => {
